...
namespace TownOfHost
{
    public static class Utils
    {
        ...
        public static void ShowHelp(byte to = 255)
        {
            ...
            + $"\n/cmd hamo - {GetString("Command.hamo")}"
            + $"\n/cmd rename(r) - {GetString("Command.rename")}"
            + $"\n/cmd rule(rl) - {GetString("Command.rule")}";
            if (!Options.OptionCommandCo.GetBool()) text += $"\n/cmd co (役職名) - {GetString("Command.co")}";
            if (!Options.OptionCommandColist.GetBool()) text += $"\n/cmd colist(cl) - {GetString("Command.colist")}";
            if (!Options.OptionCommandAco.GetBool()) text += $"\n/cmd aco (属性名) - {GetString("Command.aco")}";
            if (!Options.OptionCommandAcl.GetBool()) text += $"\n/cmd acl - {GetString("Command.acl")}";
        ...
        public static void ApplySuffix(PlayerControl pc, bool force = false, bool countdown = false)
        {
                ...
                if (!Iscountdown)
                {
                    switch (Options.GetSuffixMode())
                    {
                        ...
                        case SuffixModes.TOH:
                            name += $"<size=75%>(<{Main.ModColor}>TOH-hm v{Main.PluginShowVersion})</color></size>";
                            break;
            ...
            if (GameStates.IsLobby && !Iscountdown && (force || (pc.name != "Player(Clone)" && pc.PlayerId != PlayerControl.LocalPlayer.PlayerId && !pc.IsModClient())))
            {
                ...
                n = "<line-height=-100%>\n<b></line-height>" + name + $"\n<line-height=-{(sb.Length is 0 ? "200" : "300")}%>\n<size=60%><line-height=0%><{Main.ModColor}>TownOfHost-hamo <#ffffff>v{Main.PluginShowVersion}<line-height=100%>{sb.ToString()}</size>\n<size=0> ";
                if (force)
