...
namespace TownOfHost
{
    ...
    class ChatCommands
    {
        ...
        public static bool Prefix(ChatController __instance)
        {
            ...
            bool hasCmdPrefixLocal = args[0] == "/cmd" && args.Length > 1;
            bool isDirectCommandLocal = args[0].StartsWith("/") && args[0] != "/cmd";

            if (text.StartsWith("/") && !text.Contains("cmd") && !isDirectCommandLocal)
            {
            ...
            if (!hasCmdPrefixLocal && !isDirectCommandLocal)
            {
                if (canceled)
                {
                ...
                return !canceled;//cmdもコマンドも無い場合は処理をしない
            }
            if (hasCmdPrefixLocal)
                args = args.Skip(1).ToArray();
            // isDirectCommandLocalの場合はargsをそのまま使う("/h"などが既にargs[0]に入っている)
            ...
            if (AmongUsClient.Instance.AmHost)
            {
                switch (args[0])
                {
                    ...
                    case "/acl":
                        ...
                        // 既存の「非ホストから未知の/cmdコマンドをホストへ届ける」仕組み
                        // (ClientSendHideMessage の隠しRPC)と同じ経路でホストへ送る。
                        if (AmongUsClient.Instance.AmHost)
                        {
                            canceled = true;
        ...
        public static void OnReceiveChat(PlayerControl player, string text, out bool canceled, bool Isclient = false)
        {
            ...
            // "/cmd h" のような「/cmd 付き」形式と、"/h" のような「/cmd 無し」形式の
            // 両方を受け付ける。
            //  - "/cmd h"  → args = ["/cmd", "h"]  → 先頭の"/cmd"を取り除いて ["h"] → "/h"
            //  - "/h"      → args = ["/h"]         → そのまま ["/h"] を使う
            // どちらでもない("/"で始まるが両方の形式に当てはまらない)場合は、
            // 今まで通りコマンド失敗メッセージを出して終了する。
            bool hasCmdPrefix = args[0] == "/cmd" && args.Length > 1;
            bool isDirectCommand = args[0].StartsWith("/") && args[0] != "/cmd";

            if (text.StartsWith("/") && !text.Contains("cmd") && !isDirectCommand)
            {
                SendMessage(GetString("Error.CommandFailed"), player.PlayerId);
            }
            if (!hasCmdPrefix && !isDirectCommand) return;//cmdもコマンドも無い場合は処理をしない
            ...
            // isDirectCommandの場合はargsをそのまま使う("/h"などが既にargs[0]に入っている)
            ...
            canceled = true;
            Logger.Info($"/cmdスイッチ到達. args[0]=\"{args[0]}\" 全args=[{string.Join(",", args)}]", "ChatCommand");
            switch (args[0])
            {
                case "/revive":
                case "/rev":
